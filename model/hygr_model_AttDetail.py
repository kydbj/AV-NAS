...
class Encoder_Cell(nn.Module):
    def __init__(self,__C):
        ...
        for i in range(2):
            op = MixedOp(__C, op_name="ModelingOperators_high")
            self.ModelingOperators_high.append(op)

        for i in range(1):
            op = MixedOp(__C, op_name="ModelingOperators_low")
            self.ModelingOperators_low.append(op)

        for i in range(2):
            op = MixedOp(__C, op_name="MonadicOperators_high")
            self.MonadicOperators_high.append(op)

        for i in range(2):
            op = MixedOp(__C, op_name="MonadicOperators_low")
            self.MonadicOperators_low.append(op)

        for i in range(2):
            op = MixedOp(__C, op_name="BinaryOperators")
...
class Interact_Cell(nn.Module):
    def __init__(self,__C):
        super(Interact_Cell, self).__init__()
        self.__C = __C

        self.InterOperators = nn.ModuleList()

        for i in range(2):
            op = MixedOp(__C, op_name="InterOperators")
...
class Fusion_Cell(nn.Module):
    def __init__(self,__C):
        ...
        for i in range(1):
            op = MixedOp(__C, op_name="FusionOperators")
...
class Final_Cell(nn.Module):
    def __init__(self, __C):
        super(Final_Cell, self).__init__()
        self.__C = __C

        self.FinalOperators = nn.ModuleList()

        for i in range(1):
            op = MixedOp(__C, op_name="FinalOperators")
...
class Beckbone_Cell(nn.Module):
    def __init__(self,__C):
        super(Beckbone_Cell, self).__init__()
        self.__C = __C

        self.Img_encoder = Encoder_Cell(self.__C)
        self.Aud_encoder = Encoder_Cell(self.__C)

        self.Interaction = Interact_Cell(self.__C)
        self.Fusion = Fusion_Cell(self.__C)
        self.Final = Final_Cell(self.__C)
