...
class Encoder_Full(nn.Module):
    ...
    def forward(self, x):
        xup1 = self.up1(x)
        xup1 = self.dropout1(xup1)  # 在up1之后应用Dropout

        xup2 = self.up2(x)
        xup2 = self.dropout2(xup2)  # 在up2之后应用Dropout
        ...
        xdown = self.dropout3(xdown)  # 在down2之后应用Dropout
...
class Net_Full(nn.Module):
    ...
    def genotype(self):
        ...
        gene_Interaction1, gene_Interaction_prob1, gene_Interaction2, gene_Interaction_prob2 = self.parse(
            alpha_Interaction, Interaction_type)
        ...
        def parse_two(alpha_param_list, type_list):
            ...
            with torch.no_grad():
                for ix, edges in enumerate(alpha_param_list):
                    ...
                    if len(soft_edges) > 2 and type not in ["MonadicOperators_high", "MonadicOperators_low",
                                                            "BinaryOperators"]:
